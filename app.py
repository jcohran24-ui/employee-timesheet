...
def admin_employee_hirequest_pdf(employee_id):
    ...
    # Replace the original ticket date with the selected week's Monday.
    overlay.setFillColor(colors.white)
    overlay.rect(353, 327, 74, 18, fill=1, stroke=0)
    overlay.setFillColor(colors.black)
    overlay.setFont('Helvetica', 9.2)
    date_text = f'{week_start.month}/{week_start.day}/{week_start.year}'
    ...
    for x, value in zip(hour_centers, daily_hours):
        ...
        overlay.drawCentredString(x, hour_baseline, display)
